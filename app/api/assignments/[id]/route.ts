...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { message: "Có lỗi xảy ra khi tải thông tin bài tập" },
      { status: 500 }
    );
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error("Update assignment error:", result.error);
      return NextResponse.json({ message: result.error }, { status: 400 });
    }
  } catch (error) {
    console.error("Update assignment error:", error);
    return NextResponse.json(
      { message: "Có lỗi xảy ra khi cập nhật bài tập", error: String(error) },
      { status: 500 }
    );
  }
}
